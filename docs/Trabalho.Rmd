...
inputPanel( # uso da biblioteca shiny para o input de dados visto no curso de dashboards
  selectInput(
    inputId = "tipo_medalha",
    label = "Tipo de Medalha",
    choices = c("Ouro", "Prata", "Bronze")
  )
)
```
...
Pelo código abaixo, veremos que o valor NA se deve à existência de um atleta medalhista de bronze, cuja idade não foi informada no *dataset*.
</div>
...
<p align = justify  style="text-indent: 3em;">Continuando nossa análise, vamos ver a idade dos atletas mais jovens.</p>

```{r sumário_dados}
#| echo=TRUE
dados_agrupados_brasil |>
  group_by(idade) |> 
  summarise(idade, nome, sexo, time, cidade, idade, modalidade, evento) |> 
  arrange(idade) |> 
  head(n=10) |> 
  kbl() |>
  kable_classic(
...
<p align = justify  style="text-indent: 3em;">Na coluna **"idade"** da tabela acima, podemos ver que o atleta mais jovem tinha 13 anos de idade.</p>
...
output$p3 <- renderPlot({
  if (input$sexoidimc == "Male") { # se selecionado a opção Male
    imc_idade_modalidade_homens <- dados_agrupados_brasil |>
      ...
      group_by(modalidade) |>
      mutate(
        imc = round((peso / (altura / 100 * altura / 100)), 3),
      ...
      annotate("text", x = 44, y = 27, label = "Sobrepeso", color = "#6E0D27") +
      annotate("text", x = 43.5, y = 35.5, label = "Obesidade Grau 1", color = "#6E0D27")
    ...
    imc_idade_modalidade_mulheres <- dados_agrupados_brasil |>
      ...
      annotate("text", x = 42, y = 18.25, label = "Abaixo do Peso", color = "#6E0D27")
...
<dav align = justify><p style="color: #7C7287">De acordo com a reportagem do Globo Esporte, Adolphe Christiano Klingelhoefer com vinte anos participou dos jogos de Paris. Nascido em 2 de maio de 1880 na França, foi filho de um vice-cônsul brasileiro. </p>

<dav align = justify><p style="color: #7C7287">Naquele tempo, o artigo 6º da Constituição de 1824 dizia que *"são cidadãos brasileiros os filhos de pai brasileiro, que estiverem em país estrangeiro em serviço do Império, embora não venham estabelecer domicílio no Brasil"*</p>

<dav align = justify><p style="color: #7C7287">De posse de sua nacionalidade brasileira, Adolphe competiu na Olimpíada de Paris nas provas de atletismo de 60 metros, 200 metros e nos 110 metros com barreira, mas não avançou para as finais em nenhuma delas.</p>

<dav align = justify><p style="color: #7C7287">Segundo o Comitê Olímpico Internacional (COI), *"um atleta brasileiro (e não o Brasil) competiu nas Olimpíadas de 1900 sob as cores de um clube francês*.</p>

<dav align = justify><p style="color: #7C7287"><p>fonte: https://interativos.globoesporte.globo.com/olimpiadas/materia/adolphe </p></dav>
...
grafico_medalhas_modalidade <- medalhas_modalidade |>
  ...
  annotate("text", x = 1958, y = 20, label = "*** Selecione acima a aba 'tabela de medalhas' para informação
           sobre este ponto") +
  annotate("rect", xmin = 2005, xmax = 2011, ymin = 13, ymax = 19, fill = "#EE304D", alpha = 0.1) +
...
medalhas_masculino_feminino <- medalhistas_brasil |>
  ...
  scale_color_manual(values = c("#00A750", "#EE304D")) +
  theme_economist() +
  tema_olimpiadas() +
  theme(
  ...
  annotate("rect", xmin = 1994, xmax = 2022, ymin = 1, ymax = 16, fill = "#EE304D", alpha = 0.1)
...
medalhas_atletas |>
  ...
  geom_point(
    shape = "circle", size = 2.5, colour = "#EE304D", alpha = 1
  ) +
  ...
  labs(
    title = "Quantidade de Medalhas por Atletas
       ",
    subtitle = "A linha amarela indica uma relação direta de proporcionalidade"
  ) +
  xlab("
  ...
  theme_tufte() +
  theme(
    panel.background = element_rect(fill = "#F3F5F7"),
